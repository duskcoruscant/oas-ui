  ...
  methods: {
    ...
    openFieldForm(field, index) {
      ...
      if (index !== -1) {
        ...
        field.type === 'enum' && (this.fieldEnumList = JSON.parse(JSON.stringify(FieldObject && FieldObject.values || [])))
        // 初始化约束条件列表
        // this.fieldConstraintsList = JSON.parse(JSON.stringify(FieldObject?.validation?.constraints || []))
        this.fieldConstraintsList = JSON.parse(JSON.stringify(FieldObject && FieldObject.validation && FieldObject.validation.constraints || []))
        // 初始化自定义属性列表
        // this.fieldPropertiesList = JSON.parse(JSON.stringify(FieldObject?.properties?.values || []))
        this.fieldPropertiesList = JSON.parse(JSON.stringify(FieldObject && FieldObject.properties && FieldObject.properties.values || []))
      } else {
        this.formFieldForm = {}
        // 初始化枚举值列表
        this.fieldEnumList = []
