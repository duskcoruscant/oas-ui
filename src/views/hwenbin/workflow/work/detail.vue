                ...
                <el-timeline-item v-for="(item,index) in flowRecordList" :key="index" :icon="setIcon(item.finishTime)" :color="setColor(item.finishTime)">
                  ...
                  <el-card class="box-card" shadow="hover">
                    <el-descriptions column="5" :labelStyle="{'font-weight': 'bold'}">
                      ...
                      <el-descriptions-item label="接收时间">{{ parseTime(item.createTime) || '-'}}</el-descriptions-item>
                      <el-descriptions-item label="办结时间">{{ parseTime(item.finishTime) || '-' }}</el-descriptions-item>
                      <el-descriptions-item label="耗时">{{ item.duration || '-'}}</el-descriptions-item>
                    </el-descriptions>
                    <template v-if="item.commentList && item.commentList.length > 0">
                      <div v-for="(comment, index) in item.commentList" :key="index">
                        <el-divider content-position="left">
                          ...
                          <el-tag type="info" effect="plain" size="mini">{{ parseTime(comment.time) }}</el-tag>
                        </el-divider>
