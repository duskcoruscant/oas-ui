<template>
  <div class="app-container">
    ...
    <el-dialog :title="process.title" :visible.sync="process.open" width="500px" append-to-body>
      <el-form :model="process.form" ref="processForm" :rules="rules" size="mini" label-width="80px">
    ...
    <el-dialog title="版本管理" :visible.sync="publish.open" width="50%" append-to-body>
      ...
      <pagination
        ...
        :total="publishTotal"
        :page.sync="publishQueryParams.pageNo"
      ...
      <el-row :gutter="24">
        <el-col :span="10" :xs="24">
          ...
          <pagination
            ...
            v-show="formTotal>0"
            :total="formTotal"
            :page.sync="formQueryParams.pageNo"
  ...
  methods: {
    ...
    getPublishList() {
      ...
      publishList(this.publishQueryParams).then(response => {
        this.publish.dataList = response.data.list
        this.publishTotal = response.data.total
    ...
    categoryFormat(row, column) {
      const item = this.categoryOptions.find(k => k.code === row.category)
      const categoryName = item && item.categoryName
      return (categoryName !== null && categoryName !== undefined) ? categoryName : ''
    }
  }
}
</script>
