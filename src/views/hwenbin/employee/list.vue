  ...
  <div class="app-container">
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="listQuery" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
          <el-form-item label="员工姓名" prop="name">
            <el-input v-model="listQuery.name" placeholder="请输入员工姓名" clearable style="width: 240px"
                      @keyup.enter.native="handleQuery"/>
          </el-form-item>
          <el-form-item label="手机号码" prop="phone">
            <el-input v-model="listQuery.phone" placeholder="请输入手机号码" clearable style="width: 240px"
                      @keyup.enter.native="handleQuery"/>
          </el-form-item>
          <el-form-item label="状态" prop="status">
            <el-select v-model="listQuery.status" placeholder="员工状态" clearable style="width: 240px">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                       v-if="hasPermission('manage:employee:add')">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="info" icon="el-icon-upload2" size="mini" @click="handleImport"
                       v-if="hasPermission('manage:employee:import')">导入</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                       v-if="hasPermission('manage:employee:export')">导出</el-button>
          </el-col>
          <right-toolbar :showSearch.sync="showSearch" @queryTable="getEmployeeList" :columns="columns"></right-toolbar>
        </el-row>

        <el-table v-loading="listLoading" :data="employeeList">
          <el-table-column label="员工id" align="center" key="id" prop="id" v-if="columns[0].visible" />
          ...
          <el-table-column label="性别" align="center" key="sex" prop="sex" v-if="columns[2].visible" >
            <template slot-scope="scope">
              <el-tag :disable-transitions="true" type="success" v-if="scope.row.sex==1">女</el-tag>
              <el-tag :disable-transitions="true" v-if="scope.row.sex==2">男</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="职位" align="center" key="positionName" prop="positionName" v-if="columns[3].visible" :show-overflow-tooltip="true" />
          <el-table-column label="部门" align="center" key="deptName" prop="deptName" v-if="columns[4].visible" :show-overflow-tooltip="true" />
          <el-table-column label="入职日期" align="center" prop="entryDate" v-if="columns[5].visible" width="160">
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.entryDate, '{y}-{m}-{d}') }}</span>
            </template>
          </el-table-column>
          <el-table-column label="手机号码" align="center" key="phone" prop="phone" v-if="columns[6].visible" width="120" />
          <el-table-column label="邮箱" align="center" key="email" prop="email" v-if="columns[7].visible" width="120" :show-overflow-tooltip="true" />
          <el-table-column label="出生日期" align="center" prop="birthday" v-if="columns[8].visible" width="160">
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.birthday, '{y}-{m}-{d}') }}</span>
            </template>
          </el-table-column>
          <el-table-column label="状态" key="status" v-if="columns[9].visible" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                         v-if="hasPermission('manage:employee:update')">修改</el-button>
              <el-dropdown  @command="(command) => handleCommand(command, scope.$index, scope.row)"
                            v-if="hasPermission('manage:employee:delete', 'manage:employee:reset-password', 'manage:employee:assign-role')">
                <span class="el-dropdown-link">
                  <i class="el-icon-d-arrow-right el-icon--right"></i>更多
                </span>
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="handleDelete" v-if="scope.row.id !== 1 && hasPermission('manage:employee:delete')" 
                    size="mini" type="text" icon="el-icon-delete">删除</el-dropdown-item>
                  <el-dropdown-item command="handleResetPwd" v-if="hasPermission('manage:employee:reset-password')"
                    size="mini" type="text" icon="el-icon-key">重置密码</el-dropdown-item>
                  <el-dropdown-item command="handleRole" v-if="hasPermission('manage:employee:assign-role')"
                    size="mini" type="text" icon="el-icon-circle-check">分配角色</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="637px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="出生日期" prop="birthday">
              <el-date-picker 
                type="date" v-model="form.birthday" placeholder="选择日期" value-format="yyyy-MM-dd"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="入职日期" prop="entryDate">
              <el-date-picker 
                type="date" v-model="form.entryDate" placeholder="选择日期" value-format="yyyy-MM-dd"
              />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="职位" prop="positionIds">
              <el-select v-model="form.positionIds" multiple placeholder="请选择">
                <el-option
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item v-if="form.id === undefined" label="账户昵称" prop="nickname">
              <el-input v-model="form.nickname" placeholder="请输入账户昵称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-if="form.id === undefined" label="账户密码" prop="password">
              <el-input v-model="form.password" placeholder="请输入账户密码" type="password" show-password />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  methods: {
    ...
    getEmployeeList() {
      ...
      empApi.getEmployeeList(this.addDateRange(this.listQuery, [
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.name + '"员工吗?').then(function() {
    ...
    handleUpdate(row) {
      ...
      empApi.getEmpDetail(id).then(response => {
        ...
        this.form.entryDate = this.parseTime(this.form.entryDate, '{y}-{m}-{d}')
        this.form.password = ''
        this.form.nickname = ''
